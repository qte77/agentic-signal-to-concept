...
| Devpost sub-source | deferred | Access/ToS individually vetted, then added *inside* `build-pattern-scanner.md`, not a new spec. |
| AI-builder-tool showcases (Bolt.new/Lovable/v0/Replit) sub-source | deferred | Same — per-platform ToS vetted first. |
| Build-in-public threads (X/Bluesky/Indie Hackers) sub-source | deferred | Same — per-platform ToS vetted first. |
